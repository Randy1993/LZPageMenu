# LZPageMenu
## 简介
LZPageMenu是一个强大的控制器分段，不仅支持多种样式的分段菜单，同时支持携带头部视图，并支持CocoaPods(尚未上传成功[捂脸])。不仅仅只是实现功能，更希望从内存和性能上做优化，例如如何保证当前图层只有一个子控制器显示、又如何完成菜单子视图的复用等。因此LZPageMenu的实现方式与主流的实现大概存在差异，但也许存在更多的问题，项目处于初期阶段，望大家不吝赐教！
